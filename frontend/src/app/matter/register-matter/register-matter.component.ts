...
export class RegisterMatterComponent implements OnInit {
  ...
  registerMatter() {
    if (!this.regisD.nameM || !this.regisD.nameT) {
      ...
      this._matterService.registerMatter(this.regisD).subscribe(
        ...
        (res) => {
          ...
          this._router.navigate(['/registerCourse']); //lo redirecciona directo al formulario de crear tarea
